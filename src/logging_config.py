...
class NetworkLogger:
    ...
    def operation_context(self, operation: str, target: str = None):
        ...
        try:
            logger.info(
                ...
                extra={"operation_start": True, "target": target, "operation_id": operation_id},
            )
            ...
            duration = time.time() - start_time
            logger.error(
                f"Failed {operation} operation after {duration:.2f}s: {e}",
                extra={
    ...
    def log_command_execution(self, command: str, host: str, result: Dict[str, Any]):
        ...
        if success:
            logger.info(
                f"Command executed successfully on {host}",
                extra={
                    "command": command,
                    "host": host,
                    "exit_code": exit_code,
    ...
    def log_network_scan(self, target: str, scan_type: str, result: Dict[str, Any]):
        ...
        if success:
            ...
            logger.info(
                f"{scan_type} scan completed for {target}",
                extra={
                    "target": target,
                    "scan_type": scan_type,
            ...
            logger.error(
                f"{scan_type} scan failed for {target}",
                extra={
                    "target": target,
                    "scan_type": scan_type,
                    "error": result.get("error", "Unknown error"),
    ...
    def log_ai_interaction(self, user_input: str, parsed_command: Dict[str, Any], success: bool):
        ...
        if success:
            logger.info(
                "AI successfully parsed user command",
                extra={
            ...
            logger.warning(
                "AI failed to parse user command",
                extra={
                    "user_input": user_input,
    ...
    def get_log_stats(self) -> Dict[str, Any]:
        ...
        stats = {"log_to_file": True, "log_directory": str(self.log_dir), "log_files": []}
