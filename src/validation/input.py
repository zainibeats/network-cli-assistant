...
def create_validation_error(
    field_name: str, value: str, error_msg: str, suggestion: str = None
) -> dict:
    """
...
def validate_network_operation_input(operation: str, **kwargs) -> Tuple[bool, Optional[dict]]:
    ...
    if "host" in kwargs:
        ...
        if not is_valid:
            validation_errors.append(
                create_validation_error("target host", kwargs["host"], error_msg)
            )
    ...
    if "network" in kwargs:
        ...
        if operation == "discover_hosts":
            ...
            if not is_valid:
                validation_errors.append(
                    create_validation_error("network", kwargs["network"], error_msg)
                )
    ...
    if "src_ip" in kwargs:
        ...
        if not is_valid:
            validation_errors.append(
                create_validation_error("source IP address", kwargs["src_ip"], error_msg)
            )
    ...
    if "dst_ip" in kwargs:
        ...
        if not is_valid:
            validation_errors.append(
                create_validation_error("destination IP address", kwargs["dst_ip"], error_msg)
            )
    ...
    if "top_ports" in kwargs:
        ...
        if not is_valid:
            validation_errors.append(
                create_validation_error("port count", kwargs["top_ports"], error_msg)
            )
        elif int(kwargs["top_ports"]) > 1000:
            validation_errors.append(
                create_validation_error(
                    "port count", kwargs["top_ports"], "Port count too high (max 1000)"
                )
            )
    ...
    if operation == "generate_acl":
        if "action" in kwargs and kwargs["action"] not in ["permit", "deny"]:
            validation_errors.append(
                create_validation_error(
                    "action", kwargs["action"], "Action must be either 'permit' or 'deny'"
                )
            )
    ...
    if operation == "run_command":
        if "cmd" in kwargs:
            ...
            if not cmd or not isinstance(cmd, str) or not cmd.strip():
                validation_errors.append(
                    create_validation_error("command", str(cmd), "Command cannot be empty")
                )
            else:
