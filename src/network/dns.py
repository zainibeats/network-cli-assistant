...
from ..validation.network import validate_ip, validate_target
...
def dns_lookup(host: str) -> dict:
    ...
    results = {"success": True, "forward_lookup": None, "reverse_lookup": None}
    ...
    try:
        if is_ip_address:
            ...
            print("Input detected as IP address, performing reverse lookup...")
            try:
                ...
                try:
                    ...
                    if forward_ip != host:
                        logger.warning(
                            f"DNS consistency issue: forward lookup returned {forward_ip}, original was {host}"
                        )
                        results["forward_lookup"]["warning"] = (
                            f"Forward lookup returned different IP ({forward_ip}) than original ({host})"
                        )
            ...
            print("Input detected as hostname, performing forward lookup...")
            try:
                ...
                try:
                    ...
                    if reverse_hostname.lower() != host.lower():
                        logger.warning(
                            f"DNS consistency issue: reverse lookup returned {reverse_hostname}, original was {host}"
                        )
                        results["reverse_lookup"]["note"] = (
                            f"Reverse lookup returned different hostname ({reverse_hostname}) than original ({host})"
                        )
