...
def interpret_nmap_results(scan_results: dict) -> dict:
    ...
    if not scan_results.get("ports_found"):
        return {
            ...
            "recommendations": [
                "Verify target is reachable",
                "System may have strong firewall protection",
            ],
        }
    ...
    if risk_counts["critical"] > 0:
        summary_parts.append(
            f"{risk_counts['critical']} CRITICAL risk service{'s' if risk_counts['critical'] != 1 else ''}"
        )
    if risk_counts["high"] > 0:
        summary_parts.append(
            f"{risk_counts['high']} high risk service{'s' if risk_counts['high'] != 1 else ''}"
        )
    if risk_counts["medium"] > 0:
        summary_parts.append(
            f"{risk_counts['medium']} medium risk service{'s' if risk_counts['medium'] != 1 else ''}"
        )
    ...
    if total_open > 10:
        recommendations.append(
            "GENERAL: Large number of open ports detected - review if all services are necessary"
        )

    if any(port in ["22", "3389"] for port in open_ports):
        recommendations.append(
            "REMOTE ACCESS: SSH/RDP detected - ensure strong authentication is enabled"
        )

    if any(port in ["80", "443", "8080", "8443"] for port in open_ports):
        recommendations.append(
            "WEB SERVICES: Web servers detected - ensure they are properly secured and updated"
        )

    if any(port in ["1433", "3306", "5432", "6379", "27017"] for port in open_ports):
        recommendations.append(
            "DATABASES: Database services exposed - these should NEVER be accessible from internet"
        )
