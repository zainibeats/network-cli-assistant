...
from ..command_runner import CommandExecutionError, run_command
...
def run_nmap_scan(
    ...
    port_options_count = sum(
        1 for x in [top_ports != 10, port_range is not None, specific_ports is not None] if x
    )
    if port_options_count > 1:
        return {
            "success": False,
            "error": "Only one port scanning option can be specified: top_ports, port_range, or specific_ports",
        }
    ...
    if specific_ports is not None:
        ...
        try:
            ...
            return {
                "success": False,
                "error": "Invalid specific ports format. Use comma-separated numbers (e.g., '22,80,443')",
            }
    ...
    if port_range is not None:
        ...
        try:
            if "-" not in port_range:
                return {
                    "success": False,
                    "error": "Port range must be in format 'start-end' (e.g., '1-1000')",
                }
            ...
            return {
                "success": False,
                "error": "Invalid port range format. Use 'start-end' (e.g., '1-1000')",
            }
    ...
    cmd.extend(
        [
            "-oX",
            "-",  # Output as XML to stdout for easier parsing
            "--host-timeout",
            f"{timeout}s",  # Per-host timeout
            "--max-rtt-timeout",
            "2s",  # Maximum round-trip time
            "--initial-rtt-timeout",
            "500ms",  # Initial RTT timeout
        ]
    )
    ...
    logger.info(
        f"Starting nmap scan of {target}",
        extra={
            "target": target,
    ...
    try:
        ...
        result = run_command(cmd, timeout=timeout + 30)
                    ...
                    for port in ports.findall("port"):
                        ...
                        if state is not None:
                            ...
                            port_info = {
                                ...
                                "security_risk": _assess_port_security_risk(
                                    port_id, port_state, service_name
                                ),
                                "recommendations": _get_port_security_recommendations(
        ...
        if not parsed_output:
            ...
            if len(all_hosts) > 1:
                results["output"] = (
                    f"Scanned {len(all_hosts)} hosts on {target} ({scan_description}) in {elapsed_time:.1f}s. No open ports found."
                )
            else:
                results["output"] = (
                    f"No open or filtered ports found on {target} ({scan_description}) in {elapsed_time:.1f}s."
                )
            results["interpretation"] = {
                ...
                "recommendations": [
                    ...
                    "Systems may have strong firewall protection",
                ],
            }
        else:
            open_ports = [p for p in parsed_output if p.get("state") == "open"]
            logger.info(
                f"Nmap scan completed: found {len(open_ports)} open ports on {len(hosts_with_ports)} hosts",
                extra={
            ...
            if len(hosts_with_ports) > 1:
                results["network_summary"] = (
                    f"Found open ports on {len(hosts_with_ports)} hosts:\n\n"
                )
                for host_data in hosts_with_ports:
                    ...
                    if host_ports:  # Only show hosts that actually have open ports
                        ...
                        for port in host_ports:
                            results["network_summary"] += (
                                f"  • Port {port['port']} ({port['service']}) - {port['security_risk'].upper()} RISK\n"
                            )
                        results["network_summary"] += "\n"
        ...
        if "Host seems down" in (e.result.stderr or ""):
            logger.warning(f"Host {target} appears to be down")
            return {
                "success": True,
                "output": f"Host {target} appears to be down or not responding to ping.",
            }
        logger.error(f"Nmap scan failed: {e.result.stderr or str(e)}")
        return {"success": False, "error": e.result.stderr or str(e), "error_type": "nmap_failed"}
        ...
        return {"success": False, "error": str(ex), "error_type": "unexpected_error"}
...
def _get_port_security_recommendations(port: str, service: str) -> list:
    ...
    recommendations = {
        "21": [
            ...
            "If FTP needed, use FTPS with encryption",
        ...
        "23": [
            "CRITICAL: Replace with SSH immediately",
            "Telnet transmits passwords in plain text",
            "Disable telnet service",
        ],
        "25": [
            "Ensure not an open relay",
            "Use SMTP authentication",
            "Enable TLS encryption",
            "Monitor for spam",
        ],
        "53": [
            "Restrict to authorized networks only",
        ...
        "139": [
            ...
            "Legacy protocol with security issues",
            "Use SMB over port 445 instead",
        ],
        "143": [
...
def run_netstat() -> dict:
    ...
    try:
        ...
        logger.info("Netstat completed successfully", extra={"exit_code": result.returncode})
        ...
        logger.error(f"Netstat failed: {e.result.stderr or str(e)}")
        return {
            "success": False,
            "error": e.result.stderr or str(e),
