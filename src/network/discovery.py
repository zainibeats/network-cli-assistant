...
def discover_hosts(
    network: str, max_hosts: int = 254, scan_method: str = "ping", timeout: int = 300
) -> dict:
    """
    ...
    if target_type != "cidr":
        return {
            "success": False,
            "error": "Host discovery requires a network range in CIDR notation (e.g., 192.168.1.0/24)",
        }
    ...
    if scan_method not in valid_methods:
        return {
            "success": False,
            "error": f"scan_method must be one of: {', '.join(valid_methods)}",
        }
    ...
    try:
        ...
        logger.info(
            f"Starting host discovery on {network}",
            extra={
                "network": network,
                "max_hosts": max_hosts,
        ...
        cmd.extend(
            [
                "--host-timeout",
                f"{min(timeout // hosts_to_scan, 30)}s",  # Per-host timeout
                "--max-rtt-timeout",
                "2s",
                "-oG",
                "-",  # Greppable output for easier parsing
            ]
        )
        ...
        for i in range(0, len(host_list), batch_size):
            ...
            try:
                result = run_command(batch_cmd, timeout=timeout)
                ...
                if hosts_to_scan > 20 and processed_hosts % 20 == 0:
                    ...
                    print(
                        f"Progress: {processed_hosts}/{hosts_to_scan} hosts scanned ({progress:.1f}%) - {len(active_hosts)} active hosts found"
                    )

            except (CommandExecutionError, subprocess.TimeoutExpired) as e:
        ...
        logger.info(
            f"Host discovery completed: found {len(unique_hosts)} active hosts",
            extra={
        ...
        if unique_hosts:
            results["output"] = (
                f"Found {len(unique_hosts)} active hosts on {network} (scanned {processed_hosts} hosts in {elapsed_time:.1f}s):\n"
            )
            ...
            results["output"] = (
                f"No active hosts found in {network} (scanned {processed_hosts} hosts in {elapsed_time:.1f}s)."
            )
            results["output"] += f"\nScan method: {scan_method.upper()}"
            results["output"] += (
        ...
        logger.error(f"Host discovery failed: {e.result.stderr or str(e)}")
