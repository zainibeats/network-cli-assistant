...
def ping(host: str) -> dict:
    ...
    try:
        ...
        logger.info(
            "Ping completed successfully", extra={"packets_sent": 4, "exit_code": result.returncode}
        )
        ...
        logger.error("Ping operation timed out after 30 seconds")
        return {
            "success": False,
            "error": "Ping operation timed out after 30 seconds",
            "error_type": "timeout",
        }
    except CommandExecutionError as e:
        stderr_output = e.result.stderr or ""

        if (
            "Name or service not known" in stderr_output
            or "cannot resolve" in stderr_output.lower()
        ):
            ...
            logger.error("Permission denied for ping operation")
            return {
                "success": False,
...
def traceroute(host: str) -> dict:
    ...
    try:
        ...
        logger.info("Traceroute completed successfully", extra={"exit_code": result.returncode})
        ...
        logger.error("Traceroute operation timed out after 120 seconds")
        return {
            "success": False,
            "error": "Traceroute operation timed out after 120 seconds",
            "error_type": "timeout",
        }
    except CommandExecutionError as e:
        stderr_output = e.result.stderr or ""
        ...
        if (
            ...
            logger.error("Permission denied for traceroute operation")
            return {
                "success": False,
