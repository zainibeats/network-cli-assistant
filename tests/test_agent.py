...
@pytest.mark.parametrize(
    "user_input",
    [
        "investigate server 192.168.1.10",
        "troubleshoot github",
        "investigate my network",
        "diagnose network 10.0.0.0/16",
        "scan localhost for vulnerabilities",
        "what network is the machine on?",
        "parse through logs for errors",
        "scan 192.168.1.20 for vulnerabilities",
        "security check network 192.168.1.0/24",
        "scan my ubuntu server for vulnerabilities",
        "find the latest docs for installing jellyfin with docker compose",
    ],
)
def test_build_agent_plan_does_not_use_keyword_triggers(user_input):
    assert build_agent_plan(user_input) is None
...
def test_execute_agent_plan_uses_only_planned_functions():
    ...
    plan = {
        ...
        "target": "192.168.1.10",
        "steps": [
            {"function": "ping", "args": {"host": "192.168.1.10"}, "reason": "Check reachability"},
            {"function": "traceroute", "args": {"host": "192.168.1.10"}, "reason": "Inspect path"},
            {
                "function": "run_nmap_scan",
                "args": {"target": "192.168.1.10", "top_ports": 10},
                "reason": "Check common exposed services",
            },
        ],
    }
    ...
    result = execute_agent_plan(
        {
            ...
            "steps": [
                {
                    "function": "run_bash",
                    "args": {"command": "ss -tulpen"},
                    "reason": "List local sockets",
                },
                {
                    "function": "run_bash",
                    "args": {"command": "ip addr show"},
                    "reason": "Review interfaces",
                },
                {
                    "function": "run_nmap_scan",
                    "args": {"target": "localhost", "top_ports": 100},
                    "reason": "Scan common ports",
                },
            ],
        },
        function_resolver=fake_resolver,
...
def test_handle_agent_message_uses_shell_planner_for_free_form_request(monkeypatch, tmp_path):
...
def test_handle_agent_message_routes_parsed_command_through_agent(monkeypatch, tmp_path):
    monkeypatch.setattr(agent, "append_chat_turn", lambda *_args, **_kwargs: tmp_path)
    monkeypatch.setattr(agent, "build_shell_agent_plan", lambda _user_input: None)
    monkeypatch.setattr(
        agent,
        "parse_command",
        lambda _user_input: {
...
def test_help_is_not_handled_as_canned_chat(monkeypatch, tmp_path):
    monkeypatch.setattr(agent, "append_chat_turn", lambda *_args, **_kwargs: tmp_path)
    monkeypatch.setattr(agent, "build_shell_agent_plan", lambda _user_input: None)
    monkeypatch.setattr(
        agent,
        ...
        lambda user_input: {
            "status": "error",
            "message": f"parser saw {user_input}",
        },
    )

    response = handle_agent_message("help")

    assert "parser saw help" in response
